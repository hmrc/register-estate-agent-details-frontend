...
class AffinityGroupIdentifierActionSpec extends PlaySpec with SpecBase {
  ...
  lazy val estatesAuth = new EstatesAuthorisedFunctions(mockAuthConnector, appConfig)
    ...
    "an Agent user" when {
      "hasn't enrolled an Agent Services Account" must {
        "redirect the user to the create agent services page" in {
          ...
          redirectLocation(result).value mustBe s"${appConfig.createAgentServicesAccountUrl}"
          application.stop()
        }
      }
